$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [{label: '游戏编号', name: 'goodsId', index: 'goodsId', width: 60, key: true}, {
            label: '游戏名', name: 'goodsName', index: 'goodsName', width: 120
        }, {label: '游戏简介', name: 'goodsIntro', index: 'goodsIntro', width: 120}, {
            label: '游戏图片', name: 'goodsCoverImg', index: 'goodsCoverImg', width: 120, formatter: coverImageFormatter
        ...
        if (flag.value) {
            $.ajax({
                ...
                success: function (r) {
                    if (r.resultCode == 200) {
                        Swal.fire({
                            text: "上架成功", icon: "success", iconColor: "#1d953f",
                        });
        ...
        if (flag.value) {
            $.ajax({
                ...
                success: function (r) {
                    if (r.resultCode == 200) {
                        Swal.fire({
                            text: "下架成功", icon: "success", iconColor: "#1d953f",
                        });
                        $("#jqGrid").trigger("reloadGrid");
                    } else {
                        Swal.fire({
